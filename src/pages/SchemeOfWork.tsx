...
import { BookOpen, Download, Plus, Trash2, GripVertical, Check } from 'lucide-react';
...
import { SAMPLE_TOPICS } from '@/lib/curriculum';
...
export default function SchemeOfWork() {
  ...
  const [weeks, setWeeks] = useState<SOWType['weeks']>([]);
  ...
  const generateWeeks = () => {
    const topics = SAMPLE_TOPICS[subject]?.[classLevel];
    const generated: SOWType['weeks'] = Array.from({ length: 13 }, (_, i) => ({
      week: i + 1,
      topic: topics?.[i]?.topic || '',
      subTopic: topics?.[i]?.subTopic || '',
      objectives: ['Students will be able to understand and apply concepts related to the topic'],
      ...
      <div className="pb-24 px-4 pt-4">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={() => setMode('list')}>Back</Button>
          </div>
          ...
          {weeks.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground">
                Edit topics below. {subject} — {classLevel} — Term {term}
              </p>
              {weeks.map((w, i) => (
    ...
    <div className="pb-24 px-4 pt-4">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => { setMode('create'); setWeeks([]); setSubject(''); setClassLevel(''); }}>
        ...
        {sows.length === 0 ? (
          <div className="glass-card rounded-2xl p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Generate a Scheme of Work to plan your term's topics across 13 weeks.
            </p>
            <Button onClick={() => { setMode('create'); setWeeks([]); }}>Create Scheme</Button>
          </div>
        ) : (
          <div className="space-y-3">
            {sows.map(sow => (
              <div key={sow.id} className="glass-card rounded-xl p-4">
                ...
                <p className="text-xs text-muted-foreground">{sow.weeks.length} weeks planned</p>
